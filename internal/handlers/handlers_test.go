...
func TestRepository_Reservation(t *testing.T) {
	...
	// Test case where reservation is not in session (should redirect)
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
	...
	// Test case where room ID doesn't exist (should redirect)
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
...
func TestRepository_PostReservation(t *testing.T) {
	...
	// Test validation failure (first name too short) - should re-render form with errors
	reqBody = "start_date=01/01/2100"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=01/02/2100")
	...
	if rr.Code != http.StatusOK {
		t.Errorf("PostReservation handler returned wrong response code for validation failure: got %d, wanted %d", rr.Code, http.StatusOK)
	}

	// Test database error (room id 2 triggers error in test repo)
	reqBody = "start_date=01/01/2100"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=01/02/2100")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	...
	// Test room restriction error (room id 1000 triggers error in test repo)
	reqBody = "start_date=01/01/2100"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=01/02/2100")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
